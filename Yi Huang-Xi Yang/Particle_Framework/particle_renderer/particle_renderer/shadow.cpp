...
static float *global_d2;

static int
compare_distances2(const void *a,
                  const void *b)
{
    int i=*(const int*)a,
        j=*(const int*)b;
    float di=global_d2[i],
          dj=global_d2[j];
    if(di<dj) return -1;
     ...
     float opacity)
{
    ...
    for(int j=jlo; j<=jhi; ++j){
        ...
        for(int i=ilo; i<=ihi; ++i){
            ...
            float t=u*u+v*v;
            if(t<r2){
                img(i,j)*=std::exp(-opacity*kernel2(t/r2));
            }
        }
    }
}
                ...
                Array2f &shadow_map)
{
    ...
    global_d2=d.data;
    qsort(order.data, n, sizeof(int), compare_distances2);
